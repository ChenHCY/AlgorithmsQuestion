...
3. 继续递归 =》根据划分的区域继续找到新的root(左右子树的根节点）左子树节点： currRoot(当前节点在preorder中的index) + 1 / 右子树节点： currRoot(当前节点在preorder中的index) + 左子树的个数(rootIndex - inStart + 1)]

4. 然后继续根据中序遍历的特性，找到这两个节点的左右子树部分，缩小区域递归 =》直到使用完所以的节点值，形成了新的二叉树
*/
...
class Solution {
    ...
    public TreeNode buildTree(int[] preorder, int[] inorder, int currRoot, int inStart, int inEnd, HashMap<Integer, Integer> map){
        ...
        //得到当前根节点root的左右子树个数，map里面存的是 每个node值 和 其在inorder[]中序遍历的index位置
        int rootIndex = map.get(preorder[currRoot]); //根据这个节点值，对应的inorder里面的index, 得到当前根节点的左右子树个数

        //2. 左子树的区域： inorder [isStart - rootIndex]， 左子树的root节点：preorder[currRoot + 1]
        root.left = buildTree(preorder, inorder, currRoot + 1, inStart, rootIndex - 1, map);

        //3. 右子树的区域： inorder [(rootIndex + 1) - isEnd]， 右子树区域的root节点：preorder[currRoot + 左子树的个数(rootIndex - inStart + 1)]
