...
class Solution {
    public List<String> findHighAccessEmployees(List<List<String>> access_times) {
        ...
        //Used HashMap save every employeed name with their acess time
        HashMap<String, List<Integer>> map = new HashMap<>(); 
        for(List<String> access : access_times){
            String staffName = access.get(0);
            int acTime = Integer.parseInt(access.get(1));
            
            map.computeIfAbsent(staffName, k -> new ArrayList<>()).add(acTime);
        }
        
        //travser all the element and count every staff whether have 3 access times in one hour
        for(Map.Entry<String, List<Integer>> entry : map.entrySet()){
            List<Integer> timeList = entry.getValue(); // get the curr Employee Time List
            Collections.sort(timeList); //sort the time in increasing order
            ...
            while(right < timeList.size()){
                if(timeList.get(right) - timeList.get(left) >= 100){
                    left++;
                }
