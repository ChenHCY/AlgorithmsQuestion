...
class ParkingSystem {
    /* parkingsystem: [big, medium, small]
       big = 1 meidum = 2 small = 3
    */
    int[] parkingSystem = new int[4]; //the slot still not park
    int[] parked = new int[4]; //the slot have pared
    
    public ParkingSystem(int big, int medium, int small) {
        //big = 1 meidum = 2 small = 3
        parkingSystem[1] = big;
        parkingSystem[2] = medium;
        parkingSystem[3] = small;
    }
    
    public boolean addCar(int carType) {
        if(parked[carType] <  parkingSystem[carType]){
            parked[carType]++; //now is parked
            return true;
        }
        
        return false; //have no slot can park
    }
}
